...
@router.post("/registration", response_description="Registration a user", response_model=UserResponse)
async def registration(user_info: User):
    ...
    if email_found:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="The email address already taken")
